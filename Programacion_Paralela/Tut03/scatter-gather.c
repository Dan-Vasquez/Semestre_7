...
int main ( int argc, char *argv[] )
{
  ...
  double *avgs;
  if ( my_rank == ROOT_PROC )
    avgs = ( double * ) malloc ( sizeof ( double ) * p );
  /* Gather all partial averages down to the root process */
  MPI_Gather ( &my_avg, 1, MPI_DOUBLE,
               avgs, 1, MPI_DOUBLE,
               ROOT_PROC, MPI_COMM_WORLD );

  /* Now that we have all of the partial averages on the root, compute the
      global average of all numbers. Since we are assuming each process
      computed an average across an equal amount of elements, this computation
  ...
  if ( my_rank == ROOT_PROC )
  {
    ...
    printf ( "Sum of local averages is %lf; global average is %lf\n",
             sum_of_avgs, global_avg );
