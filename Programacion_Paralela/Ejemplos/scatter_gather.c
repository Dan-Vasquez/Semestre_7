...
avgs, 1, MPI_DOUBLE,
MPI_COMM_WORLD );
/* Now that we have all of the partial averages, compute the global
average of all numbers.
Since we are assuming each process computed an average across an
equal amount of elements, this computation will produce the
correct answer. */
double sum_of_avgs = 0;
for ( int rank = 0; rank < p; rank++ )
	sum_of_avgs += avgs [rank];
double global_avg = sum_of_avgs / p;
...
if ( my_rank == ROOT_PROC )
{
	double global_sum = (double) num_elements * ( num_elements + 1 ) / 2;
